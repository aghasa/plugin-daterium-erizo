...
class Metodos_Daterium
{
    ...
    /**
     * Función para obtener la lista de categorias inferiores.
     */
    public function daterium_get_list_categories($xml)
    {
        $i = 1;
        $filas = [];
        foreach ($xml->xpath('arbol/paso') as $nodo) {
            $caja = [
                'pos' => $i++,
                'tipo' => 'cat',
                'id' => intval($nodo->catID),
                'nombre' => strval($nodo->nombre),
                'imagen' => strval($nodo->img500x500),
                'descripcion' => strval($nodo->descripcion),
                'extra' => '',
                'url' => site_url() . '/' . basename(get_permalink(get_the_ID())) . '/' . intval($nodo->catID) . '/' . $this->daterium_url_title($nodo->nombre),
                'activo' => true,
                'novedad' => false,
                'promocionado' => false,
                'proximamente' => false,
     ...
     */
    public function daterium_get_list_products($xml)
    {
        $i = 1;
        $filas = [];
     ...
     */
    public function get_nombre_categoria($xml)
    {
        $nombre = $xml->nombre;
